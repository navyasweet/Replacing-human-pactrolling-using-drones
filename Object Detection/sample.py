...
# Load your fighting detection model (replace 'fighting_detection_model.h5' with your model file)
fighting_model = load_model('fighting_detection_model.h5')
...
# Define a function to perform fighting detection
def detect_fighting(frame):
    # Preprocess the frame (resize, normalize, etc.) as needed
    processed_frame = preprocess_frame(frame)
    
    # Use the fighting detection model to predict
    prediction = fighting_model.predict(np.expand_dims(processed_frame, axis=0))
    
    # Check if fighting is detected based on the model's output
    if prediction > 0.5:  # You may need to adjust the threshold
        return True
    else:
        return False
